...
// PhonePlugin 支付宝手机网站支付插件
// 参考 Python 的 AlipayPhonePluginResponder，继承自 AlipayWapPluginResponder
type PhonePlugin struct {
	*BasePlugin // 嵌入 BasePlugin（支付宝基类），可以使用支付宝通用功能
}

// NewPhonePlugin 创建支付宝手机网站支付插件
...
func (p *PhonePlugin) CreateOrder(ctx context.Context, req *plugin.CreateOrderRequest) (*plugin.CreateOrderResponse, error) {
	...
	if req.DetailID > 0 {
		...
		if err := database.DB.Where("id = ?", req.DetailID).First(&orderDetail).Error; err != nil {
			return plugin.NewErrorResponse(7320, "订单详情不存在"), nil
		}
	} else if req.OrderID != "" {
		...
		return plugin.NewErrorResponse(7320, "缺少订单ID或详情ID"), nil
	}
	...
	if req.OrderID != "" {
		if err := database.DB.Where("id = ?", req.OrderID).First(&order).Error; err != nil {
			return plugin.NewErrorResponse(7320, "订单不存在"), nil
		}
	} else if req.OrderNo != "" {
		...
		return plugin.NewErrorResponse(7320, "缺少订单ID或订单号"), nil
	}
...
func (p *PhonePlugin) generatePayURL(ctx context.Context, req *plugin.CreateOrderRequest, productID string, order *models.Order) (string, error) {
...
func (p *PhonePlugin) generateSubject(req *plugin.CreateOrderRequest) string {
	// 从产品表获取 subject
...
func (p *PhonePlugin) buildAlipayWapPayURL(req *plugin.CreateOrderRequest, domainURL, notifyURL, subject, productID string, order *models.Order) (string, error) {
	// 解析产品ID
...
var _ plugin.PluginCapabilities = (*PhonePlugin)(nil)

// CanHandleExtra 是否可以处理额外参数
func (p *PhonePlugin) CanHandleExtra() bool {
	return false
}

// AutoExtra 是否自动处理额外参数
func (p *PhonePlugin) AutoExtra() bool {
	return false
}

// ExtraNeedProduct 额外参数是否需要产品
func (p *PhonePlugin) ExtraNeedProduct() bool {
	return false
}

// ExtraNeedCookie 额外参数是否需要Cookie
func (p *PhonePlugin) ExtraNeedCookie() bool {
	return false
}

// GetTimeout 获取订单超时时间（秒）
func (p *PhonePlugin) GetTimeout(ctx context.Context, pluginID int64) int {
	// 默认5分钟
	return 300
}
