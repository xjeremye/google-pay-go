...
// AppPlugin 支付宝APP支付插件
// 参考 Python: AlipayAppPluginResponder
type AppPlugin struct {
	*BasePlugin // 嵌入 BasePlugin（支付宝基类）
}

// NewAppPlugin 创建支付宝APP支付插件
func NewAppPlugin(pluginID int64) *AppPlugin {
	return &AppPlugin{
...
// CreateOrder 创建订单
// 参考 Python: AlipayAppPluginResponder.create_order
func (p *AppPlugin) CreateOrder(ctx context.Context, req *plugin.CreateOrderRequest) (*plugin.CreateOrderResponse, error) {
	// 获取订单详情和订单信息
...
func (p *AppPlugin) getOrderInfo(req *plugin.CreateOrderRequest) (*models.OrderDetail, *models.Order, error) {
...
// 参考 Python: AlipayAppPluginResponder.get_pay_url
func (p *AppPlugin) generatePayURL(ctx context.Context, req *plugin.CreateOrderRequest, productID string, order *models.Order) (string, error) {
	// 获取域名信息
	...
	// 构建通知URL
	notifyURL := fmt.Sprintf("%s/api/pay/order/notify/alipay_app/%s/", notifyDomain, productID)

	// 生成订单主题
	subject := generateSubject(ctx, req, productID)

	// 构建支付URL（APP支付返回查询字符串）
	payURL, err := p.buildAlipayAppPayURL(req, notifyURL, subject, productID)
	if err != nil {
		return "", err
	}
...
// buildAlipayAppPayURL 构建支付宝APP支付URL（返回查询字符串）
func (p *AppPlugin) buildAlipayAppPayURL(req *plugin.CreateOrderRequest, notifyURL, subject, productID string) (string, error) {
	// 解析产品ID
	...
	// 调用支付宝 SDK 生成支付查询字符串
	queryString, err := alipayClient.TradeAppPay(subject, req.OrderNo, totalAmount, notifyURL, others)
	if err != nil {
		return "", fmt.Errorf("生成支付URL失败: %w", err)
	}

	// APP支付返回查询字符串，客户端需要自行处理
	// 参考 Python: get_alipay_app_url 可能会根据设备类型处理
	// 这里简化处理，直接返回查询字符串
	return queryString, nil
}

// 实现 PluginCapabilities 接口
var _ plugin.PluginCapabilities = (*AppPlugin)(nil)

// CanHandleExtra 是否可以处理额外参数
func (p *AppPlugin) CanHandleExtra() bool {
	return false
}

// AutoExtra 是否自动处理额外参数
func (p *AppPlugin) AutoExtra() bool {
	return false
}

// ExtraNeedProduct 额外参数是否需要产品
func (p *AppPlugin) ExtraNeedProduct() bool {
	return false
}

// ExtraNeedCookie 额外参数是否需要Cookie
func (p *AppPlugin) ExtraNeedCookie() bool {
	return false
}

// GetTimeout 获取订单超时时间（秒）
func (p *AppPlugin) GetTimeout(ctx context.Context, pluginID int64) int {
	return 300
}
